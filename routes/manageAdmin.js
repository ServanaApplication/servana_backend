...
router.get('/', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('system_user')
      .select('sys_user_id, sys_user_email, sys_user_is_active, supabase_user_id') // ✅ Removed password
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (sys_user_email || sys_user_password) {
      ...
      if (sys_user_password) updates.password = sys_user_password;

      const { error: authUpdateErr } = await supabase.auth.admin.updateUserById(supabaseUserId, updates);
